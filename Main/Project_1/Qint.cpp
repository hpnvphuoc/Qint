...
string ConvertHexCharToBin(char src) {//chuyen ki tu thap luc phan sang nhi phan
	switch (src) {
	case '0': return "0000";
	case '1': return "0001";
	case '2': return "0010";
...
string QInt::StrHextoBin(string src){
	string result;
	string temp;
	for (int i = 0; i < src.length(); i++) {
		temp = ConvertHexCharToBin(src[i]);
		result += temp;
	}
	return result;
}
